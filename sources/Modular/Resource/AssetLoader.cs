...
public class AssetLoader {
	public delegate void UrlProcessor(Identifier id, Url assetUrl);
	...
	public virtual void Next() {
		...
		if (_processedCount == 0) {
			BeginTask();
			foreach (AssetLoader c in _subordinateLoaders) {
				c.BeginTask();
			}
		}
		// Final stage.
		if (_taskQueue.Count == 0) {
			...
			AssetLoader subLoader = _subordinateLoaders.Peek();
			subLoader.Next();
			++_processedCount;
