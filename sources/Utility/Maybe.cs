...
public readonly struct Maybe<T> : IEquatable<Maybe<T>> {
	...
	/// <exception cref="ArgumentException">Thrown if input is null.</exception>
    public static Maybe<T> Of(in T value) {
		if (default(T) is null && value is null) {
			throw new ArgumentException("Cannot create non-none maybe with null value");
		}
	...
	/// </summary>
	/// <exception cref="NullReferenceException">Thrown if this is none.</exception>
    public T Value {
		get {
			if (!HasValue) {
				throw new NullReferenceException("Maybe has no value");
			}
			return _value!;
		}
	}
